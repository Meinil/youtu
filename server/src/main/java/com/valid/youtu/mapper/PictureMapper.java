...
public interface PictureMapper extends BaseMapper<Picture> {
    @Select("SELECT p.id, p.name, p.create_time, p.story FROM picture p INNER JOIN classify c ON p.cid=c.id WHERE c.name=#{name} LIMIT #{page}, #{total}")
    List<Picture> getPicturesPagesByClassify(@Param("page") Integer page, @Param("total") Integer total, @Param("name")String name);

    @Select("SELECT p.id, p.name, p.create_time, p.story FROM picture p INNER JOIN classify c ON p.cid=c.id LIMIT #{page}, #{total}")
